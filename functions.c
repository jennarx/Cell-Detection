...
// Function to test binary erosion on "example.bmp"
void test_binary_erosion(){
    printf("Running test_binary_erosion...\n");

    // Allocate memory for images on the heap
    unsigned char (*input_image)[BMP_HEIGTH][BMP_CHANNELS] = malloc(BMP_WIDTH * BMP_HEIGTH * BMP_CHANNELS);
    unsigned char (*output_image1)[BMP_HEIGTH][BMP_CHANNELS] = malloc(BMP_WIDTH * BMP_HEIGTH * BMP_CHANNELS);
    unsigned char (*output_image2)[BMP_HEIGTH][BMP_CHANNELS] = malloc(BMP_WIDTH * BMP_HEIGTH * BMP_CHANNELS);
    unsigned char (*output_image3)[BMP_HEIGTH][BMP_CHANNELS] = malloc(BMP_WIDTH * BMP_HEIGTH * BMP_CHANNELS);
    unsigned char (*expected_image)[BMP_HEIGTH][BMP_CHANNELS] = malloc(BMP_WIDTH * BMP_HEIGTH * BMP_CHANNELS);

    if (!input_image || !output_image1 || !output_image2 || !output_image3 || !expected_image) {
        printf("Memory allocation failed\n");
        exit(1);
    }
    ...
    // Run the greyscale and binary threshold functions to prepare the image for binary erosion
    greyscale(input_image, output_image1);
    binary_threshold(output_image1, output_image2);

    // Perform binary erosion on the image that was prepared
    binary_erosion(output_image2, output_image3);

    // Test case where images are equal
    // Initialize expected_image that should be all black
    for (int x = 0; x < BMP_WIDTH; x++) {
        for (int y = 0; y < BMP_HEIGTH; y++) {
            expected_image[x][y][0] = 0;
            expected_image[x][y][1] = 0;
            expected_image[x][y][2] = 0;
        }
    }

    // Assert equality
    assert_equal(output_image3, expected_image, "test_binary_erosion_equal");

    // Test case where images are not equal
    // Modify expected_image to make it different
    for (int x = 0; x < BMP_WIDTH; x++) {
        for (int y = 0; y < BMP_HEIGTH; y++) {
            expected_image[x][y][0] = 255;
            expected_image[x][y][1] = 255;
            expected_image[x][y][2] = 255;
        }
    }
    // Assert inequality
    assert_equal(output_image3, expected_image, "test_binary_erosion_not_equal");
    ...
    printf("Completed test_binary_erosion.\n");
}